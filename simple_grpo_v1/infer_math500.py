...
SYSTEM_PROMPT = (
    ...
    # "Take a brief thought and give a concie reply. Don't think too long."
...
def build_retry_prompt(tokenizer: AutoTokenizer, question: str, first_ans: str) -> Tuple[str, torch.Tensor]:
    tip_text = tokenizer.apply_chat_template(
        [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": question},
            {"role": "assistant", "content": first_ans},
            {"role": "user", "content": "Your previous answer was incorrect. Check the previous steps. If you know where you went wrong, solve it correctly. Otherwise, try a different approach to solve the problem."},
        ],
        tokenize=False,
        add_generation_prompt=True,
    )
...
def extract_answer(text: str) -> str | None:
    ...
    if match:
        ...
        boxed = extract_boxed_answer(text, False)
        return boxed if boxed else text
    return None


def normalize_answer(ans: str) -> str:
    """Normalize latex/symbolic answers to comparable form."""
    ans = ans.strip()
    ...
    # \frac{a}{b}
    ans = re.sub(r"\\frac\s*\{\s*([^{}]+?)\s*\}\s*\{\s*([^{}]+?)\s*\}", r"(\1)/(\2)", ans)
...
def extract_boxed_answer(text: str, last_num: bool) -> str | None:
...
def check_correct(pred_text: str, gt_text: str) -> bool:
    ...
    if pred is None:
        pred = extract_boxed_answer(pred_text, True)
        if pred is None:
            return False
    ...
    try:
        ans = parse(pred_norm, extraction_config=[ExprExtractionConfig()])
        gt_ans = parse(gt_norm, extraction_config=[ExprExtractionConfig()])
        if verify(ans, gt_ans):
            return True
    except Exception:
        pass
...
def main():
    ...
    parser.add_argument("--dataset_path", required=True, help="Path to a local MATH500 dataset saved with datasets.save_to_disk")
    ...
    parser.add_argument("--max_new_tokens", type=int, default=1536, help="Max new tokens for generation")
    ...
    print(f"Loaded MATH500 {args.split} split with {len(dataset)} samples.")
    ...
    level_score = {i : [] for i in range(1, 6)}

    for i in pbar:
        # print(dataset[i])
        question = dataset[i]["problem"]
        gt = dataset[i]["answer"].strip()
        lv = dataset[i]["level"]
        ...
        if len(outputs) == 1:
            ...
            # print(f"pred_text: {pred_text}, gt: {gt}")

            # Metrics
            if check_correct(pred_text, gt):
                correct += 1
                level_score[lv].append(1)
            else:
                level_score[lv].append(0)
                # tip_text, input_ids = build_retry_prompt(tokenizer, question, pred_text)
                # with torch.inference_mode():
                #     outputs = model.generate(
                #         input_ids.to(args.device),
                #         generation_config=retry_config,
                #     )
                # gen_ids = outputs[0, input_ids.shape[1]:]  # Only the generated part
                # retry_pred_text = tokenizer.decode(gen_ids, skip_special_tokens=True).strip()
                # if check_correct(retry_pred_text, gt):
                #     retry_correct += 1
                #     print(f"Retry correct for question {i}: {question}")
                #     print(f"  First answer: {pred_text}")
                #     print(f"  Retry answer: {retry_pred_text}")
            ...
            for pk in pks:
                ...
                pass_k[pk] += score
                scores[pk] = round(score, 4)
            pass_at_k = {pk: round(pass_k[pk]/(i+1), 4) for pk in pks}


        if i < save_rollout:
            rollouts[i] = {"question": question, "gt": gt} #, "pass@k": scores}
            results = []
        ...
        pbar.set_postfix({"correct": f'{correct:.4f}'}) #, "retry": f'{retry_correct:.4f}'}) #, "pass@k": pass_at_k})
    ...
    level_acc = {}
    for lv, scores in level_score.items():
        if len(scores) > 0:
            level_acc[lv] = sum(scores) / len(scores)
        else:
            level_acc[lv] = 0.0

    print("\n===== MATH500 Evaluation Result =====")
    print(f"Model: {args.model_path}")
    print(f"Split: {args.split}")
    print(f"Samples: {total}")
    print(f"Accuracy: {accuracy*100:.2f}% ({correct}/{total})")
    print(f"Retry Accuracy: {retry_correct*100:.2f}% ({retry_correct}/{total})")
    ...
    result = {
        ...
        "level_counts": {lv: len(scores) for lv, scores in level_score.items()},
    }
    ...
    if save_rollout > 0:  
        import json
        import os
        output_file_path = os.path.join(args.model_path, "saved_rollouts_math500_skill.json")
