...
parser.add_argument("--tokenizer_path", default=None, help="Tokenizer path; defaults to resolved --model_path")
...
parser.add_argument("--all_steps", type=int, default=20)
parser.add_argument("--save_steps", type=int, default=30)
parser.add_argument("--save_dir", type=str, default="66")
...
if os.path.exists(args.model_path + str(pre_step)):
    model_path = args.model_path + str(pre_step)
elif os.path.exists(args.model_path + "temp"):
    model_path = args.model_path + "temp"
else:
    model_path = args.model_path
print(args.model_path + str(pre_step), os.path.exists(args.model_path + str(pre_step)), model_path)
beta = 0.04
num_pre_Q = 8
...
def reward_rarity(rewards):
    ...
    if torch.all((rarity_scores - rarity_scores.mean()) == 0).item():
        return rarity_scores - rarity_scores.mean()
...
def generate_mode(num=10, rank=0):
    ...
    for ii in range(num):
        ...
        rewards = reward_rarity(rewards)
        if torch.all(rewards == 0).item():
            continue
...
for step in progress:
    ...
    while batch is None:
        batch = get_batch()
        if batch is None:
            generate_mode(rank=torch.distributed.get_rank(), num = min(10, 2*(all_steps - step + 1)))
        # dist.barrier()
    # dist.barrier()
    ...
    if step % save_steps == 0:
        ...
        if torch.distributed.get_rank() == 0:
            print('saving model')
            save_name = os.path.join(run_output_dir, "100rollout_head")
